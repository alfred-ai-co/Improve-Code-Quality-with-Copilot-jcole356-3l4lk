...
async def create_kanban_board(kanban_board: KanbanBoardCreate, db: AsyncSession = Depends(get_db)):
    ...
    return await kanban_board_crud.create(kanban_board)
...
async def update_kanban_board(id: int, kanban_board: KanbanBoardUpdate, db: AsyncSession = Depends(get_db)):
    kanban_board_crud = KanbanBoardCRUD(db)
    await kanban_board_crud.update(id, kanban_board)
    return await kanban_board_crud.get(id)


@router.delete("/{id}", status_code=204)
async def delete_kanban_board(id: int, db: AsyncSession = Depends(get_db)):
    kanban_board_crud = KanbanBoardCRUD(db)
    await kanban_board_crud.delete(id)
