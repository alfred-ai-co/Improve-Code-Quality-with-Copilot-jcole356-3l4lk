...
from app.api_models.kanbanstatus import KanbanStatusCreate, KanbanStatusResponse, KanbanStatusUpdate
...
async def create_kanban_status(kanban_status: KanbanStatusCreate, db: AsyncSession = Depends(get_db)):
    ...
    return await kanban_status_crud.create(kanban_status)
...
async def update_kanban_status(id: int, kanban_status: KanbanStatusUpdate, db: AsyncSession = Depends(get_db)):
    kanban_status_crud = KanbanStatusCRUD(db)
    await kanban_status_crud.update(id, kanban_status)
