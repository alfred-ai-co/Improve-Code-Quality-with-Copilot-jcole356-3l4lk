...
# Create a new APIRouter instance for ticket-related routes
router = APIRouter()
...
def create_ticket(ticket: TicketCreate, db: Session = Depends(get_db)):
    ...
    :param ticket: TicketCreate - Pydantic model containing ticket data
    :param db: Session - SQLAlchemy session dependency
    :return: TicketResponse - Pydantic model containing the created ticket data
    """
...
def get_all_tickets(db: Session = Depends(get_db)):
    ...
    :param db: Session - SQLAlchemy session dependency
    :return: List[TicketResponse] - List of Pydantic models containing ticket data
    """
    ticket_crud = TicketCRUD(db)
...
def get_ticket(id: int, db: Session = Depends(get_db)):
    ...
    :param db: Session - SQLAlchemy session dependency
    :return: TicketResponse - Pydantic model containing the ticket data
    :raises HTTPException: If the ticket with the given ID is not found
    """
    ticket_crud = TicketCRUD(db)
...
def update_ticket(id: int, ticket: TicketCreate, db: Session = Depends(get_db)):
    ...
    :param id: int - Ticket ID
    :param ticket: TicketCreate - Pydantic model containing updated ticket data
    :param db: Session - SQLAlchemy session dependency
    :return: TicketResponse - Pydantic model containing the updated ticket data
    """
    ticket_crud = TicketCRUD(db)
